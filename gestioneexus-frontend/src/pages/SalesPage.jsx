...
const SalesPage = () => {
    ...
    // 1. Calcula el subtotal cada vez que el carrito cambia
    useEffect(() => {
        const newSubtotal = cart.reduce((sum, item) => sum + parseFloat(item.total), 0);
        setSubtotal(newSubtotal);
    }, [cart]);

    // 2. Calcula el total final cada vez que el subtotal o el descuento cambian
    useEffect(() => {
    ...
    // 3. Calcula el cambio a devolver cada vez que el monto pagado o el total final cambian
    useEffect(() => {
    ...
    // Busca un producto por referencia y lo añade al carrito
    const handleSearchAndAdd = async (e) => {
        e.preventDefault();
        if (!searchTerm.trim()) return;

        try {
            const { data: product } = await api.get(`/products/reference/${searchTerm.trim()}`);
            ...
            if (existingItemIndex > -1) {
                ...
                if (currentItem.quantity + 1 > product.quantity) {
                     Swal.fire('Stock Insuficiente', `No puedes agregar más unidades de ${product.name}. Stock disponible: ${product.quantity}`, 'warning');
                     return;
                }
                ...
                setCart([...cart, { 
                    ...
                    quantity: 1, 
                    total: parseFloat(product.price)  // El total inicial es el precio
                }]);
            }
            setSearchTerm('');
    ...
    const handlePay = async () => {
        ...
        if (isNaN(given) || given < total) {
            Swal.fire({
                ...
                text: 'El monto dado por el cliente debe ser mayor o igual al total de la venta.',
